...
import { generateNomorPeserta, generateVA } from "@/lib/format";

export const Route = createFileRoute("/_authenticated/siswa/biodata")({
...
function BiodataPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (siswa) {
        const { error } = await supabase.from("siswa").update(parsed.data).eq("id", siswa.id);
        ...
        const nomor_va = generateVA("8888", parsed.data.no_hp || "");
        await supabase.from("pembayaran").insert({
          siswa_id: newSiswa.id, nomor_va, biaya, status: "pending",
        });
      }
