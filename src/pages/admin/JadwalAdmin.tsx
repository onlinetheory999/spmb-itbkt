...
export default function JadwalAdmin() {
  ...
  const [form, setForm] = useState<{
    judul: string; tanggal: string; tanggal_selesai: string; waktu: string;
    jenjang: "SD" | "SMP" | "SMA" | ""; deskripsi: string;
  }>({
    judul: "", tanggal: "", tanggal_selesai: "", waktu: "", jenjang: "", deskripsi: "",
  });
  ...
  async function add() {
    if (!form.judul || !form.tanggal) return toast.error("Lengkapi judul dan tanggal mulai");
    setSaving(true);
    const { error } = await supabase.from("jadwal").insert({
      judul: form.judul, tanggal: form.tanggal,
      tanggal_selesai: form.tanggal_selesai || null,
      waktu: form.waktu || null, deskripsi: form.deskripsi || null,
      jenjang: form.jenjang || null,
    } as any);
    setSaving(false);
    if (error) return toast.error(error.message);
    toast.success("Jadwal ditambahkan");
    setForm({ judul: "", tanggal: "", tanggal_selesai: "", waktu: "", jenjang: "", deskripsi: "" });
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <Card className="p-5 grid sm:grid-cols-2 gap-3">
        ...
        <div><Label>Tanggal Mulai *</Label><Input type="date" value={form.tanggal} onChange={(e) => setForm({ ...form, tanggal: e.target.value })} /></div>
        <div><Label>Tanggal Selesai (opsional)</Label><Input type="date" value={form.tanggal_selesai} onChange={(e) => setForm({ ...form, tanggal_selesai: e.target.value })} /></div>
        ...
        <div>
          ...
          <Select value={form.jenjang || "all"} onValueChange={(v) => setForm({ ...form, jenjang: v === "all" ? "" : v as any })}>
